...
int swStripedWord(unsigned char *querySeq2,
				  int queryLength,
...
int swStripedByte(unsigned char *querySeq2,
				  int queryLength,
				  unsigned char *dbSeq2,
				  int dbLength,
				  unsigned short gapOpen,
...
int align(int **dis, char *read, char *qname1, char *quality1)
{
	int maxMatch = 0;
	int tempMatch;
	...
	fprintf(outFile, "%s\t%d\t%s\t%d\t%s\t%s\t%s\tIH:i:1\tMD:Z:%s\n", qname1, flag, contigNames[maxContig], pos, cigar, read, quality1, md);
	// fprintf(outFile, "%s\t%d\t%d\t%s\t%d\t%s\tIH:i:1\tMD:Z:%s\n", qname2, flag2, pos2, cigar2, -tlen, read2, md2);
...
void *map(void *threadid)
{
	...
	char qname1[maxReadLength + 10], read1[maxReadLength + 10], quality1[maxReadLength + 10];
	// char *line = new char[MAX_REC_LEN];
	char *line = new char[maxReadLength + 10];
	...
	while (fgets(line, maxReadLength + 10, readFile) != NULL && toAlign > 0)
	{
		strcpy(qname1, line + 1);
		// fgets(read1, MAX_FILE_READ, readFile);
		fgets(read1, maxReadLength + 10, readFile);
		// fgets(line, MAX_FILE_READ, readFile);
		fgets(line, maxReadLength + 10, readFile);
		// fgets(quality1, MAX_FILE_READ, readFile);
		fgets(quality1, maxReadLength + 10, readFile);

		pthread_mutex_unlock(&readFile_mutex);
		if (strlen(read1) > MAX_FILE_READ)
			read1[MAX_FILE_READ - 1] = '\0';
		else
			read1[strlen(read1) - 1] = '\0';
		// read2[strlen(read2) - 1] = '\0';
		if (strlen(qname1) > MAX_FILE_READ)
			qname1[MAX_FILE_READ - 1] = '\0';
		else
			qname1[strlen(qname1) - 1] = '\0';

		if (strlen(quality1) > MAX_FILE_READ)
			quality1[MAX_FILE_READ - 1] = '\0';
		else
			quality1[strlen(quality1) - 1] = '\0';

		// pthread_mutex_lock(&print_mutex);
		// printf("\n*************************\n");
		// printf("read length: %d first = %c\n", strlen(read1), read1[0]);
		// printf("qname length: %d first = %c\n", strlen(qname1), qname1[0]);
		// printf("quality length: %d first = %c\n", strlen(quality1), quality1[0]);
		// printf("*************************\n");
		...
		if (rnd < toAlign / (double)totalReads)
		{
			...
			align(dis, read1, qname1, quality1);
		}
...
void printHelp()
{
	...
	cout << "----------------" << endl;
	cout << endl;
	cout << "align - maps reads not mapped by alignment tool" << endl;
	cout << "Usage:" << endl;
	cout << "align [options] <contigFile.fasta> <numberReadsToAlign> [numberThreads]" << endl;
	cout << endl;
	cout << "Required arguments:" << endl;
	cout << "<contigFile.fasta>\t Assembly file in fasta format" << endl;
	cout << "<numberReadsToAlign>\t Number of reads to align" << endl;
	cout << endl;
	cout << "Optional arguments:" << endl;
	cout << "[numberThreads]\t\t Number of threads. Default 2" << endl;
	cout << endl;
	cout << "Options:" << endl;
...
int main()
{
	...
	contigFileName = "E:\\Studies\\4-1\\CSE 400\\drive\\outputs\\canu\\real_SAMN10819805_pacbio_00_canu_v1.9.fasta";
	printf("Number of reads to align : ");
	scanf("%d", &toAlign);

	NUM_THREADS = 2;

	readFileName = "modifiedRandomSliceUnmapped.txt";
	...
	printf("Lengths of Contigs : %d\n", contigLengths[0]);

	fscanf(summaryFile, "%ld %ld %d", &totalReadCount, &totalReads, &maxReadLength);
